...
function attachQuadStream (context, res) {
  res.quadStream = () => {
    ...
    if (res.headers.has('link')) {
      const rawLink = res.headers.raw()['link'].filter((link) => {
        return link.indexOf('rel="http://www.w3.org/ns/json-ld#context"') !== -1
      }).shift()

      if (rawLink) {
        contextLinkUrl = url.resolve(res.url, linkRegExp.exec(rawLink).pop())
      }
    }

    return Promise.resolve().then(() => {
      if (contextLinkUrl) {
        return context.fetch(contextLinkUrl).then((res) => {
          return res.json()
        })
      } else {
        return undefined
